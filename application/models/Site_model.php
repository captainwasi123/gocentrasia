...
class Site_model extends CI_Model{
	...
	{
		$query = $this->db->query('SELECT country.*, GROUP_CONCAT(country_images.img) AS c_img FROM country 
		JOIN country_images ON country_images.country_id = country.id WHERE country.id = '.$id.'
		');
	 	return $query->row();
  ...
  //       INNER JOIN package_images ON package_images.pkg_id = packages.id
	...
	public function get_country_limit($table,$limit = null)
    {
        $this->db->from($table);
		$this->db->limit($limit);
	...
	public function price_range_filter($data)
	{
		$this->db->select('event.*, packages.package_name, packages.places_description, packages.description, packages.images, GROUP_CONCAT(activities.activity_name) AS activity_name, GROUP_CONCAT(activities.activity_img) AS activity_img');
		$this->db->from('event');
		$this->db->join('packages', 'event.package_id = packages.id');
		$this->db->join('package_activities', 'event.package_id = package_activities.pkg_id');
		$this->db->join('activities', 'package_activities.activity_id = activities.id');
		if (!empty($data['min_price']) && !empty('max_price')) {
			$this->db->where('group_price BETWEEN '.$data["min_price"].' AND '.$data["max_price"].'');
			// $this->db->where('group_price',$data['min_price'],'OR','private_price',$data['min_price'],'AND','group_price',$data['max_price'],'OR','private_price',$data['max_price']);
		}
		//elseif(!empty($data['act_id'])){
		// 	$this->db->where_in('package_activities.activity_id',$data['act_id']);
		// }
