                    ...
                    <div class="col-lg-12 col-12 m-t-35" style="margin-top: 0px !important;">
                            <div class="card">
                                <div class="card-header bg-white">
                                    <h5><?=$form_title?></h5>
                                     <form action="<?php echo site_url('Dashboard/save_guide'); ?>" id="add_product" method="post" enctype="multipart/form-data">
								...
								<?php foreach($edit as $value){ ?>
                                    <div class="row">
										...
										<div class="col-lg-4 input_field_sections">
                                            <h5>Guide Country<span style="color: #F00">*</span></h5>
											<select name="country" class="form-control" id="country" require>
												<option value="">select</option>
												<?php foreach ($country as $key => $cont) { ?>
													<option value="<?=$cont->id?>" 
													<?php
														if ($cont->id == $value->country_id) {
															echo "Selected";
														}
													?>
													><?=$cont->country_name?></option>
												<?php }?>
											</select>
                                        </div>
                                        <div class="col-lg-4 input_field_sections">
                                            <h5>Guide Name<span style="color: #F00">*</span></h5>
                                            <input type="text" class="form-control" name="name"
                                            placeholder="Enter Guide Name" 
											 autocomplete="off" value="<?= $value->name ?>" required/>
                                        </div>
										<div class="col-lg-4 input_field_sections">
                                            <h5>Guide Email<span style="color: #F00">*</span></h5>
                                            <input type="text" class="form-control" name="email"
                                            placeholder="Enter Guide Email" value="<?= $value->email ?>"
											 autocomplete="off" required/>
                                        </div>
                                    </div>
								<?php } ?>
                                     <br>
                                    <input class="btn btn-success glow_button" type="submit" value="Update">
                                </div>
                                       ...
                                       <tbody>
											...
											foreach ($data as $key => $value) { ?>
                                                <tr>
                                                    ...
                                                    <td><?=$value->email?></td>
                                                    <td colspan="3">
                                                        <a id="editBtn" class="btn btn-info btn-sm" href="<?=base_url();?>Dashboard/guide_edit/<?=hashids_encrypt($value->id)?>">Edit</a>
														<a id="editBtn" class="btn btn-danger btn-sm" href="<?=base_url();?>Dashboard/guide_delete/<?=hashids_encrypt($value->id)?>"><i class="fa fa-trash"></i></a>
                                                    </a>
                                                    </td>
                                                </tr>
                                            <?php }
