                            ...
                            <div class="card">
                                <div class="card-header bg-white">
                                    <h5><?=$form_title . $title?></h5>
                                     <form action="<?php echo site_url('Dashboard/departure_city_save'); ?>" id="add_product" method="post" enctype="multipart/form-data">
                                <div class="card-block">
									<?php foreach ($edit as $key => $ed) { ?>
                                    ...
                                    <div class="row">
										...
										<div class="col-lg-4 input_field_sections">
                                            <h5>Departure City<span style="color: #F00">*</span></h5>
                                            <select class="form-control" name="departure_city" id="departure_city" require>
												<option value="">Select</option>
												<?php foreach ($city as $key => $value) { ?>
													<option value="<?=$value->id?>"
													<?php if($ed->departure_city_id == $value->id){
														echo 'Selected';
													} ?>
													><?=$value->name?></option>
												<?php } ?>
                                       ...
                                       <tbody>
											...
											foreach ($data as $key => $value) { ?>
                                                <tr>
                                                    ...
                                                    <td colspan="3">
                                                        <a id="editBtn" class="btn btn-info btn-sm" href="<?=base_url();?>Dashboard/departure_city_edit/<?=hashids_encrypt($value->id)?>">Edit</a>
														<a id="editBtn" class="btn btn-danger btn-sm" href="<?=base_url();?>Dashboard/delete_departure_city/<?=hashids_encrypt($value->id)?>"><i class="fa fa-trash"></i></a>
                                                    </a>
                                                    </td>
                                                </tr>
                                            <?php }
