                    ...
                    <div class="col-lg-12 col-12 m-t-35" style="margin-top: 0px !important;">
                            <div class="card">
                                <div class="card-header bg-white">
                                    <h5><?=$form_title?></h5>
                                     <form action="<?php echo site_url('Dashboard/update_activities'); ?>" id="add_product" method="post" enctype="multipart/form-data">
                                <div class="card-block">
                                    ...
                                    <input type="hidden" name="c_id" value="<?=$value->id?>">
                                        <div class="col-lg-6 input_field_sections">
                                            ...
                                            placeholder="Enter Country Name" value="<?=$value->activity_name?>" autocomplete="off" required/>
                                        </div>
                                        <div class="col-lg-6 input_field_sections">
                                            <h5>Activity Image<span style="color: #F00">*</span></h5>
                                            <input type="file" class="form-control" name="img" <?= $is_edit ? '' : 'required' ?> />
                                            <input type="hidden" value="<?=@$value->activity_img?>"name="old_img">
                                        </div> 
										<div class="col-6 input_field_sections">
												<h5>This activity should be used in the custom tour ?<span style="color: #F00">*</span></h5>
											    <label class="custom-control custom-radio signin_radio1">
                                                    <input id="yes" name="custom_tour_activity" type="radio" class="custom-control-input" value="1" <?php if($value->custom_tour_activity == 1){ echo "checked"; } ?>>
                                                    <span class="custom-control-indicator"></span>
                                                    <span class="custom-control-description">Yes</span>
                                                </label>
                                                <label class="custom-control custom-radio signin_radio2">
                                                    <input id="no" name="custom_tour_activity" type="radio" class="custom-control-input" value="0" <?php if($value->custom_tour_activity == 0){ echo "checked"; } ?>>
                                                    <span class="custom-control-indicator"></span>
										 ...
										 <div class="col-lg-6 input_field_sections" id="show1">
                                            ...
                                            <input type="file" id="custom_activity" class="form-control" name="custom_activity_img"
											 >
											<input type="hidden" value="<?=@$value->custom_activity_img?>"name="old_custom_activity_img">
                                        </div> 
                                        <div class="col-lg-6 input_field_sections" id="show2">
                                            <h5>Active Activity Image<span style="color: #F00">*</span></h5>
                                            <input type="file" id="active_activity" class="form-control" name="true_activity_img" 
											
											/>
											<input type="hidden" value="<?=@$value->true_activity_img?>"name="old_true_activity_img" >
                                        </div> 
                                    <?php } ?>
                                    </div>
                                    </div>
...
if(no.attr("checked")){
	...
	var yesradio = $("#yes").click(function(){
		$("#show1").show();
		$("#show2").show();
		$("#active_activity").prop('required',true);;
		$("#custom_activity").prop('required',true);;
	})
</script>
