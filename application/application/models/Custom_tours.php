...
class Custom_tours extends CI_Model{
	...
	{
		$query = $this->db->query('SELECT custom_tour.*, GROUP_CONCAT(activities.activity_name) AS activity_name, GROUP_CONCAT(activities.activity_img) AS activity_img, GROUP_CONCAT(activities.id) AS act_id FROM `custom_tour` JOIN custom_tour_activities ON custom_tour.id = custom_tour_activities.custom_id JOIN activities ON activities.id = custom_tour_activities.activity_id GROUP by custom_tour.id'
		);
		return $query->result();
	}
